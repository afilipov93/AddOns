...
function ATSW_HideWindow()
	...
	if(AC_Craft and AC_Craft:GetParent()==ATSWFrame and atsw_oldmode==false and TradeSkillFrame~=nil) then -- for ArmorCraft
...
function ATSW_HideBlizzardCraftFrame()
	CraftFrame:SetAlpha(0);
	CraftFrame:ClearAllPoints();
	CraftFrame:SetPoint("TOPLEFT", 0, 900);
end
...
function ATSW_ShowBlizzardCraftFrame()
...
function ATSWFrame_OnEvent()
	...
	if(CraftFrame and CraftFrame:IsVisible() and ATSW_IsCurrentlyEnabled() and GetCraftDisplaySkillLine()) then
		ATSW_HideBlizzardCraftFrame();
	end
	if(event=="TRADE_SKILL_UPDATE") then
		...
		if(atsw_scans<2) then
			...
			if(ATSW_GetTradeSkillSelectionIndex()>0) then
				ATSWFrame_SetSelection(ATSW_GetTradeSkillSelectionIndex());
			else
				if(ATSW_GetNumTradeSkills()>0) then
					ATSWFrame_SetSelection(ATSW_GetFirstTradeSkill());
	...
	for i=1, select("#", ...), 1 do
		if (atsw_currentsubclassfilter[atsw_selectedskill] and atsw_currentsubclassfilter[atsw_selectedskill]==0 and select("#", ...)>1) then
			...
			if(atsw_currentsubclassfilter[atsw_selectedskill] and i==atsw_currentsubclassfilter[atsw_selectedskill]) then
				checked=true;
				UIDropDownMenu_SetText(select(i, ...), ATSWSubClassDropDown);
			else
				checked=false;
			end
		end
	...
	for i=1, select("#", ...), 1 do
		if (atsw_currentinvslotfilter[atsw_selectedskill] and atsw_currentinvslotfilter[atsw_selectedskill]==0 and select("#", ...)>1) then
			...
			if(atsw_currentinvslotfilter[atsw_selectedskill] and i==atsw_currentinvslotfilter[atsw_selectedskill]) then
				checked=true;
				UIDropDownMenu_SetText(select(i, ...), ATSWInvSlotDropDown);
			else
				checked=false;
			end
		end
...
function ATSW_GetLinkForSkill(skillname)
	for i=1,ATSW_GetNumTradeSkills(),1 do
		local skill=ATSW_GetTradeSkillInfo(i)
		if(skill==skillname) then
			return ATSW_GetTradeSkillItemLink(i);
		end
	end
	return nil;
end
		...
		if(skillName~=nil) then
			if(skillType=="header") then
				...
				if(currentHeader>0 or atsw_oldmode) then
					...
					local recipeLink=ATSW_GetTradeSkillRecipeLink(i);
...
function ATSWAuction_UpdateReagentList()
	...
	for i=1,5,1 do
		...
		if(necessary[offset+i]) then
			...
			inv:Disable();
			inv:Show();
			if(items_inventory>=necessary[offset+i].count) then
				inv:SetDisabledTextColor(0,1,0);
			else
				inv:SetDisabledTextColor(1,0,0);
			end
			...
			bank:Disable();
			bank:Show();
			if(items_bank>=necessary[offset+i].count) then
				bank:SetDisabledTextColor(0,1,0);
			else
				bank:SetDisabledTextColor(1,0,0);
			end
			...
			alt:Show();
			if(items_alt>=necessary[offset+i].count) then
				alt:SetDisabledTextColor(0,1,0);
			else
				alt:SetDisabledTextColor(1,0,0);
			end
...
atsw_scan_numtradeskills=0;
atsw_scan_nextskill=0;
...
function ATSW_InitSlowScan()
	...
	local blah=ATSW_GetNumTradeSkills();
...
function ATSWScanDelayFrame_OnUpdate()
	...
	if(atsw_scan_timeout<=0) then
		...
		if(atsw_scan_state==1) then
			atsw_scan_state=2;
			atsw_scan_numtradeskills=ATSW_GetNumTradeSkills();
...
function ATSW_ContinueSlowScan()
	if(atsw_scan_nextskill>=atsw_scan_numtradeskills) then
		...
		local skillName, skillType, numAvailable, isExpanded = ATSW_GetTradeSkillInfo(atsw_scan_nextskill);
		if(skillName~=nil) then
			if(skillType~="header") then
				local numReagents = ATSW_GetTradeSkillNumReagents(atsw_scan_nextskill);
				local skillLink = ATSW_GetTradeSkillItemLink(atsw_scan_nextskill);
				local numMade = ATSW_GetTradeSkillNumMade(atsw_scan_nextskill);
				for j=1, numReagents, 1 do
					local reagentName, reagentTexture, reagentCount, playerReagentCount = ATSW_GetTradeSkillReagentInfo(atsw_scan_nextskill, j);
					local reagentLink = ATSW_GetTradeSkillReagentItemLink(atsw_scan_nextskill,j);
				end
			end
		end
